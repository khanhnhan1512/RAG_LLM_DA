...
from stages.stage_1_learn_rules_from_data.rule_learning import rules_statistics
from stages.stage_b_rule_application import main_rule_application as ra
from stages.stage_c_reasoning.score_function import score_12, score_13, score_14
from params import str_to_bool
from utils import load_json_data, get_win_subgraph
        ...
        if test_query[1] in rules_dict:
            ...
            if cands_dict[0]:
                for s in range(len(args)):
                    ...
                    scores = [np.float(x) for x in scores]
...
def main():
    ...
    new_rules_dict, sort_rules_dict = ra.filter_rules(
        rules_dict, min_conf=parsed["min_conf"], min_body_supp=2, rule_lengths=parsed["rule_lengths"]
    )
